<footer class="bg-slate-50 flex flex-col items-center p-8 shadow-inner text-center">
    <div class="border flex flex-col mb-4 md:flex-row md:w-auto text-gray-700 w-full">
        @foreach ($links as $name => $link)
            <a href="{{ $link }}" class="{{ Route::is($name) ? 'bg-gray-700 hover:bg-gray-800 text-white' : 'bg-white hover:bg-gray-400 hover:text-white' }} flex items-center justify-center px-4 py-2">
